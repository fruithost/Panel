...
?>
    <form method="post"
          action="<?php print $this->url('/admin/users'.(empty($tab) ? '' : sprintf('/%s', $tab)).(empty($action) ? '' : sprintf('/%s', $action))); ?>">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb"
                    ...
                    <li class="breadcrumb-item<?php print (empty($submodule) ? ' active" aria-current="page' : ''); ?>">
                        <a class="active"
                           href="<?php print $this->url('/admin/users'); ?>"><?php I18N::__('Users'); ?></a>
                    </li>
					<?php
					...
					if(is_numeric($tab) && $tab > 0) {
							...
							<?php
								if(Auth::hasPermission('USERS::EDIT')) {
                                    ...
                                    <button type="submit" name="action" value="save"
                                            class="btn btn-sm btn-outline-primary"><?php I18N::__('Save'); ?></button>
                        ...
                        <div class="btn-group mr-2">
                            <button type="submit" name="action" value="create"
                                    class="btn btn-sm btn-outline-primary"><?php I18N::__('Create'); ?></button>
                        </div>
						<?php
					} else {
						?>
                        <div class="btn-group mr-2">
							...
							<?php
								if(Auth::hasPermission('USERS::CREATE')) {
                                    ...
                                    <a href="<?php print $this->url('/admin/users/create'); ?>"
                                       class="btn btn-sm btn-outline-primary"><?php I18N::__('Add new'); ?></a>
									<?php
								}
								if(Auth::hasPermission('USERS::DELETE')) {
									?>
                                    <button type="submit" name="action" value="deletes"
                                            class="btn btn-sm btn-outline-danger"
                                            data-confirm="<?php I18N::__('Do you really wan\'t to delete all selected users?'); ?>"><?php I18N::__('Delete'); ?></button>
