<?php
	$template->header();
	?>
	<form class="form-password text-center" method="post" action="<?php print $template->url(true); ?>">
				...
				if(isset($changeable) && $changeable) {
					?>
						<div class="form-label-group text-left">
							<input type="password" id="password_new" name="password_new" class="form-control" placeholder="New Password" required autofocus />
							<label for="password_new">New Password</label>
						</div>
						<div class="form-label-group text-left">
							<input type="password" id="password_repeated" name="password_repeated" class="form-control" placeholder="Password Confirmation" required autofocus />
							<label for="password_repeated">Password Confirmation</label>
						</div>
					<?php
				} else {
					?>
						<div class="form-label-group text-left">
							<input type="email" id="email" name="email" class="form-control" placeholder="E-Mail" required autofocus value="<?php print $email; ?>" />
		...
		<div class="checkbox mb-3 mt-3 row text-left">
			<a class="col-12 text-left" href="<?php print $template->url('/login'); ?>"><i class="material-icons">arrow_left</i> Back to the login</a>
		</div>
		<?php
			if(!isset($success)) {
				?>
					<button class="btn btn-lg btn-primary btn-block" name="action" value="lost-password" type="submit">Reset Password</button>
				<?php
			}
		?>
