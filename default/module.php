...
?>
	<form method="post" action="<?php print $this->url(true); ?>">
		<header class="<?php
            print implode(' ', $this->getCore()->getHooks()->applyFilter('MODULES_PAGE_CLASSES', [
                'page-header',
                'd-flex',
							...
							<?php
								foreach($buttons AS $button) {
									if(is_array($button) && !is_object($button)) {
										printf('<div class="btn-group mx-2">');
                                        ...
                                        if($button instanceof Button) {
											if($button->hasModal()) {
												printf('<button type="button" name="%1$s" data-bs-toggle="modal" data-bs-target="#%4$s" class="btn mx-2 btn-sm %3$s">%2$s</button>', $button->getName(), $button->getLabel(), $button->getClasses(false), $button->getModal());
											} else {
												printf('<button type="submit" name="action" value="%1$s" class="btn mx-2 btn-sm %3$s">%2$s</button>', $button->getName(), $button->getLabel(), $button->getClasses(false));
											}
										}
									}
								}
