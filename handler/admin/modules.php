<?php
	...
	$template->getAdminCore()->addModal((new Modal('add_repository', I18N::get('Add Repository'), 'admin/modules/repository/create'))->addButton([ (new Button())->setName('cancel')->setLabel(I18N::get('Cancel'))->addClass('btn-outline-danger')->setDismissable(), (new Button())->setName('create')->setLabel(I18N::get('Create'))->addClass('btn-outline-success') ])->onSave(function(array $data = []) : string | bool {
	...
	if(isset($_GET['settings'])) {
		...
		if($modules->hasModule($_GET['disable'], true)) {
			...
			if(!$module->isEnabled()) {
				...
				Database::update(DATABASE_PREFIX.'modules', 'name', [ 'name' => $module->getDirectory(), 'state' => 'DISABLED' ]);
		...
		if($modules->hasModule($_GET['enable'], true)) {
			...
			if($module->isEnabled()) {
				...
				Database::update(DATABASE_PREFIX.'modules', 'name', [ 'name' => $module->getDirectory(), 'state' => 'ENABLED' ]);
						...
						if(file_put_contents($zip_path, $zip)) {
							...
							if($zip->open($zip_path) !== true) {
								...
								if(file_exists(sprintf('%s/setup/install.php', $module_path))) {
									...
									try {
										...
										if($root) {
											...
											$php = new PHP();
											$php->setPath(PATH);
											$php->execute('classes/System/Loader.class.php', [ 'DAEMON' => true, 'REQUEST_URI' => '/', 'MODULE' => sprintf('%s/setup/install.php', $module_path) ]);
								...
								if($finished) {
									...
									Database::insert(DATABASE_PREFIX.'modules', [ 'id' => null, 'name' => $found, 'state' => 'DISABLED', 'time_enabled' => null, 'time_updated' => null, 'time_deleted' => null ]);
		...
		if($modules->hasModule($_GET['deinstall'], true)) {
			...
			Database::update(DATABASE_PREFIX.'modules', 'name', [ 'name' => $module->getDirectory(), 'state' => 'DISABLED', 'time_deleted' => date('Y-m-d H:i:s', time()), ]);
			if($module->isEnabled()) {
				$module->setEnabled(false);
			}
